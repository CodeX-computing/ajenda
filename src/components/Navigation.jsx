...
function Navigation() {
  ...
  return (
    <nav className="z-10 absolute top-0 h-20 w-screen flex justify-between items-center bg-white/50 shadow-2xl shadow-gray-100">
      <Link to="/">
        <img src={logo} alt="logo" className="logo h-16" />
      </Link>
      <ul className={`sm:${isOpened ? '-translate-x-0' : '-translate-y-full translate-x-full'} duration-300 flex flex-col absolute h-screen bg-gray-800 lg:bg-gray-700/0 backdrop-blur-50 text-white lg:text-gray-900 p-32 lg:p-4 top-0 bottom-10 right-0 rounded-bl-full lg:rounded-none justify-start lg:justify-end items-start lg:flex-row lg:translate-x-0 z-10`}>
      ...
      <button onClick={() => toggleMenu(!isOpened)} className="absolute m-4 right-0 flex flex-col items-center items-end justify-center w-10 h-10 bg-white/90 shadow shadow-xl shadow-gray-200 hover:shadow-inner rounded-full cursor-pointer lg:hidden hover:bg-gray-200 z-10" type="button">
